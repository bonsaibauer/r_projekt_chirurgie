...
html_output_regression <- file.path(output_dir, "regressionstabelle.html")
capture.output(
  stargazer(reg1, reg2, reg3, type = "html", 
            title = "Regressionstabelle: Komplikationsrisiko ~ Blutverlust / Operationsdauer",
            out = html_output_regression), 
  file = NULL  # Verhindert die Ausgabe in der Konsole
)
...
cat("----------------------------------------- \n")
cat("----------------------------------------- \n")
cat("✅ Nach welcher Methode folgt die Korrelationsanalyse (Pearson/Spearman/Kendall): \n")
